			 ...
			 umode_t mode, bool want_excl)
{
	...
	struct fs_struct *copied_fs;
    	...
    	if(!check_caller_access_to_name(dir, dentry->d_name, 1)) {
		err = -EACCES;
		goto out_eacces;
	}
	...
	current->fs->umask = 0;
	err = vfs_create2(lower_dentry_mnt, lower_parent_dentry->d_inode, lower_dentry, mode, true);
	if (err)
		goto out;
			 ...
			 struct inode *new_dir, struct dentry *new_dentry)
{
	...
	if (!check_caller_access_to_name(old_dir, &old_dentry->d_name, 1) ||
		!check_caller_access_to_name(new_dir, &new_dentry->d_name, 1)) {
		err = -EACCES;
		goto out_eacces;
	}
